    ...
    getWoeid(latitude,longitude).then(data => {
      ...
      .then(woeid => {
         ...
         getWeatherAW(woeid).then(data => {
            ...
            tod.textContent = `Min Temp: ${today.min_temp.toFixed(2)}°C Max Temp: ${today.max_temp.toFixed(2)}°C ${today.weather_state_name} `;
            todEmojiEl.textContent = todEmoji;
            tom.textContent = `Min Temp: ${tomorrow.min_temp.toFixed(2)}°C Max Temp: ${tomorrow.max_temp.toFixed(2)}°C ${tomorrow.weather_state_name} `;
            tomEmojiEl.textContent = tomEmoji;
            dat.textContent = `Min Temp: ${dayAfterTomorrow.min_temp.toFixed(2)}°C Max Temp: ${dayAfterTomorrow.max_temp.toFixed(2)}°C ${dayAfterTomorrow.weather_state_name} `;
